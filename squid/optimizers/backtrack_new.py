...
def backtrack(target_function, fval, old_fval, new_gradient,
    ...
    if (target_function is not None and
        ...
        # When backtracking, we slow down by step_size_adjustment
        # HOWEVER! If we are > ALPHA_CONST (our starting step size),
        # we simply jump to ALPHA_CONST
        if step_size > ALPHA_CONST:
    ...
    # Thus, we reset to the original step_size
        ...
        if reset_step_size < 0 and step_size < ALPHA_CONST:
            # If we have accelerate on, then slowly grow back to ALPHA_CONST
            if accelerate and linesearch is not None:
