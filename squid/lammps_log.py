"""
The lammps_log code is code from the pizza.py toolkit (www.cs.sandia.gov/~sjplimp/pizza.html) developed
by Steve Plimpton (sjplimp@sandia.gov) with some additional warning interspersed through the thermo
output.
...
#   12/15, Yaset Acevedo: Allows for warnings interspersed through the thermo output
...
class lammps_log:
	...
	**Parameters**
		...
		list: *str*
			Path to the lammps log file that is to be parsed. Note, several files
			can be included in this string as long as they are separated by spaces.
		read_all: *int, optional*
			If this is set to 0, don't read in the whole file upon initialization.
			This lets you use the :func:`next` functionality.
	...
	**Contains**
		...
		ptr: *dict*
			Dictionary corresponding the thermo keys to which column of the output they
			reside in. ptr[thermo_key] = which column this data is in
	...
	**Returns**
	...
	def next(self):
		"""
		Read the next line of thermo information from the file. Note, this is used
		when two arguments are passed during initialization.
	...
	def read_one(self,*list):
		...
		while not last:
			...
			# Check if there is an warning message on the line prior to "Loop time of"
