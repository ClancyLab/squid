...
*Angle*, *Dihedral*, and *Improper* objects.
...
class _Physical(object):
    """
    An instance of Physical is a physical object or collection of physical
    objects, such as a Molecule, Atom, Bond, Dihedral, System, etc.
    This class is designed as a parent class to Atom, Molecule, Bond,
    System, and Dihedral to provide a basic equals() method and other
    shared methods.

    Invariant: Everything about a _Physical instance must be described
    by attributes whose order does not vary or whose attributes' equivalence
    is not defined by their order (e.g. Sets). No _Physical instance may
    have a 2-D list as an attribute, or equals() will fail.
    """

    def __eq__(self, other):
        """
        Basic equivalence of self and other if they have the same pointer id,
        or if all non-callable dir entries of the two objects are equivalent.

        Since several parameters are not preserved between intuitively
        equivalent _Physical instances (e.g. Atom.index or
        Atom.molecule_index, Angle.theta because of how Angles are added to
        a System), and these must be explicitly discounted in this method.
        ...
        compared instead.
        """
        # Check basic identifiers to see if the two are obviously
        # equal or inequal
        if id(self) == id(other):
            return True
        ...
        # Compare all attributes of the two objects. If they are exactly
        ...
        for a in selfDict:
            ...
            # Passed-on attributes which are not conserved between atoms
                ...
                continue
            elif isinstance(self, Atom) and a in ["index", "molecule_index"]:
                continue
            ...
            # Checking _Physical attributes.
                ...
                # If an attribute is a list or tuple, go through it
                # element-by-element, assuming the order is the same
                # between other and self, and if any of the lists's
                # elements are _Physical instances, compare them with
                # .equals() Exception: If self is an Atom, do not
                # compare the bonded lists. This would result in an
                # infinite loop.
            elif (isinstance(selfDict[a], list) or
                  isinstance(selfDict[a], tuple)):
                if len(selfDict[a]) != len(otherDict[a]):
                    return False
...
class Atom(_Physical):
    ...
    **Returns**
    ...
    def __init__(self, element, x, y, z,
                 index=None, type=None,
                 molecule_index=1, label=None):
    ...
    def _check_3d_vec(v):
        """
        Given what should be a vector of 3 coordinates, we ensure that they
        are indeed valid.
        """
        assert hasattr(v, "__len__"), "Error - A vector was expected."
        assert len(v) == 3, "Error - A 3D vector was expected."
        assert all(map(_is_float, v)), "Error - Non-numerical values passed."
    ...
    def _to_string(self, verbose=False):
        values = (
            self.molecule_index, self.index,
            self.x, self.y, self.z,
            self.element, self.label
        )
        return '''molecule_index: %d and index: %d
    x, y, z = (%3.3f, %3.3f, %3.3f)
    element = %s and label = %s
''' % values
...
class Molecule(_Physical):
    ...
    **Returns**
    ...
    def net_charge(self):
        ...
        """
        return sum([a.charge for a in self.atoms if hasattr(a, "charge")])
    ...
    def perturbate(self, dx=0.1, dr=5, center_of_geometry=True, rotate=True):
        ...
        **Returns**
            ...
            None
        """
        raise Exception("THIS SHOULD BE MOVED TO GEOMETRY MODULE")
        for a in self.atoms:
...
set_positions.")
    ...
    def get_center_of_geometry(self, skip_H=False):
        ...
        **Returns**
            ...
            cog: *tuple, float*
                A tuple of the x, y, and z coordinate
    ...
    def merge(self, other):
        """
        This function merges another molecule into this
        one, offsetting indices as needed.
        """
